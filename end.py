...
def goodend():
    ...
    pygame.mixer.Sound.set_volume(background_music,1)
    ...
    for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
    frase01 = fonte.render("You move on with your life", 1, WHITE)
    SCREEN.blit(frase01, (100,80))
    frase02 = fonte.render("You're happy... For now",1, WHITE)
...
def badend():
    ...
    for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
    frase01 = fonte.render("You couldn't take it anymore", 1, WHITE)
    SCREEN.blit(frase01, (100,80))
    frase02 = fonte.render("Unfortanely... It's the end for you",1, WHITE)
    SCREEN.blit(frase02, (100,110))
    pygame.display.update()
    grito1 = pygame.mixer.Sound('audio\grito 01.mp3')
    pygame.mixer.Sound.play(grito1)
    pygame.mixer.Sound.set_volume(grito1,0.5)
    pygame.time.wait(3000)
    depre = 0
    v.weekloop = False
    v.start = True
